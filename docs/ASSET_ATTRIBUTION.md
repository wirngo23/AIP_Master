...
The additional `hair_segmenter.tflite` is Google's two-class Hair Segmenter, float32 version 1: [model asset](https://storage.googleapis.com/mediapipe-models/image_segmenter/hair_segmenter/float32/1/hair_segmenter.tflite). The [model card](https://storage.googleapis.com/mediapipe-assets/Model%20Card%20-%20Hair%20Segmentation.pdf) specifies Apache 2.0 and requests citation of A. Tkachenka et al., *Real-time Hair segmentation and recoloring on Mobile GPUs*, CVPR Workshop on Computer Vision for Augmented and Virtual Reality, 2019. The Apache license is included in the same directory. Hair masking runs locally and is used only for optional color exploration.
