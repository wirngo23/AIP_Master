...
- A later portal-switch check exposed a nonfatal ResizeObserver delivery-loop notice. The clinical reference now schedules backing-buffer resizing in an animation frame, skips unchanged dimensions and leaves CSS sizing alone. Retesting portal remounts, maximum beard density and reference rotation after reload did not reproduce the notice; all 36 tests and TypeScript still pass.
